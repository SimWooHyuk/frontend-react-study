...
function ApiRequest(props) {
  ...
  const handleRequestById = (id) => {
    ...
    axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`)
      ...
      .catch((err) => { // 요청 실패한 경우 에러 핸들링
        console.log(err);
      })
  }

  // 2.async/await
  const handleRequestByAsync = async (id) => {
    try {
  ...
  return (
    <>
      <div>
        <button type='button' onClick={() => handleRequestByAsync(44)}>불러오기</button>
        <button type='button' onClick={() => practice1(44)}>불러오기</button>
        <button type='button' onClick={() => practice2(44)}>불러오기</button>
      </div>
      {data &&
        <>
