...
export const counterSlice = createSlice({
  ...
  reducers: {
    ...
    incrementByPlus: (state, action) => { // 두번째 파라미터:  action 객체 = { type: 발생한 액션의 타입, payload: 화물, 전달 데이터}
      console.log(action);
      state.value += action.payload
    },
    incrementByMinus: (state, action) => { // 두번째 파라미터:  action 객체 = { type: 발생한 액션의 타입, payload: 화물, 전달 데이터}
      console.log(action);
      state.value -= action.payload
    },
    incrementByMulti: (state, action) => { // 두번째 파라미터:  action 객체 = { type: 발생한 액션의 타입, payload: 화물, 전달 데이터}
      console.log(action);
      state.value *= action.payload
    },
    incrementByDivide: (state, action) => { // 두번째 파라미터:  action 객체 = { type: 발생한 액션의 타입, payload: 화물, 전달 데이터}
      console.log(action);
      state.value /= action.payload
    },
    incrementByZero: (state, action) => { // 두번째 파라미터:  action 객체 = { type: 발생한 액션의 타입, payload: 화물, 전달 데이터}
      console.log(action);
      state.value = 0
    },
  },
...
export const { increment, decrement , incrementByPlus , incrementByMinus ,incrementByMulti, incrementByDivide, incrementByZero} = counterSlice.actions;
